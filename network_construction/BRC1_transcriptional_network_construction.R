...
network.representation <- read.table(file="brc1_transcriptional_network.tsv",header=T,sep = "\t",as.is = T,quote = "",comment.char = "")
head(network.representation)

network.genes <- network.representation$names

tn.regulation <- network.representation[,21:57]
rownames(tn.regulation) <- network.genes
head(tn.regulation)
tn.regulation[,] <- 0

tf.files <- paste0(colnames(network.representation)[21:57],".txt")

for(i in 1:length(tf.files))
{
  current.targets <- read.table(file = paste0("tf_targets/",tf.files[i]),header = T,as.is=T)[[2]]
  
  if(sum(!(current.targets %in% network.genes)) != 0)
  {
    print("Warning!!!!")
    print(current.targets[which(!(current.targets %in% network.genes))])
    break()
  }
  
  tn.regulation[current.targets,i] <- 1
}

network.representation[,21:57] <- tn.regulation

write.table(network.representation, file="brc1_transcriptional_network.1.tsv", 
            sep = "\t", 
            quote = FALSE,
            row.names = FALSE)

tn.regulation["AT3G18550",]
network.representation <- read.table(file="brc1_transcriptional_network.1.tsv",header=T,sep = "\t",as.is = T,quote = "",comment.char = "")
head(network.representation)
