...
#  observeEvent(input$plot_click, {
  ...
  observeEvent(input$button_tfs, {
    ...
    selected.genes.df <- network.data[selected.genes,]
    ...
    if(input$draw_venn_diagram)
    {
      ...
      {
        venn.list <- rep( list(list()), length(input$selected.tfs)+1 ) 
      }
      ...
      } else
      {
        ...
        venn.plot <- venn.diagram(venn.list, filename = NULL, alpha=c(0.5,0.5,0.5), cex = 2, 
    ...
    if(input$overlap_test)
    {
      if(input$selected_cluster == "None")
      {
        sets <- vector(mode = "list",length = length(input$selected.tfs))
      } else
      {
      ...
      for(i in 1:length(input$selected.tfs))
      {
        ...
        print(current.tf)
        sets[[i]] <- gene.names[network.data[,current.tf] == 1]
      }
      
      if(input$selected_cluster != "None")
      {
        sets[[length(input$selected.tfs) + 1]] <- cluster
      }
      
      results.table <- summary(supertest(x = sets, n = 931))
      p.value <- tail(results.table$P.value, n=1) #Get the last p-value
      enrichment <- (results.table$Table)[["FE"]][nrow(results.table$Table)]
