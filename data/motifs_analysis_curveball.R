##############################
# motifs_analysis_curveball.R
##############################
# This is a modification of the script motifs_analysis.R
# to randomize the network following the curveball algorithm
# as implemented in: 
# Strona, Giovanni, et al. Nature communications 5.1 (2014): 4114.
#
...
brc1.adjlist <- get.adjlist(brc1.graph, mode = "out")
...
write(occurrency.subgraph.three.nodes.in.brc1,file="motifs/occurency_subgraph_three_nodes_in_brc1_curveball.txt",ncolumns = 1)
...
for (i in 1:number.randomisation)
{
 ...
 sample <- curveball.randomise.dir(brc1.adjlist, Ncurve, T, T)
 current.random.adjacency <- sample[[Ncurve]]
 current.random.network <- graph.adjlist(current.random.adjacency)
...
write.table(x = motifs.3.random.graph,file = "motifs/motifs_three_random_graph_curveball.txt",quote = F,row.names = F,col.names = F,sep = "\t")
write.table(x = moments.random.graph,file = "motifs/random_networks_moments_curveball.txt",quote = F,row.names = F,col.names = F,sep = "\t")

random.network.moments <- read.table(file = "motifs/random_networks_moments_curveball.txt",header = T)
head(random.network.moments)
summary(random.network.moments[[3]])
summary(random.network.moments[[5]])

motifs.3.random.graph <- read.table(file="motifs/motifs_three_random_graph_curveball.txt",header=F,as.is=T)
...
estimated.p.values.high <- vector(mode="numeric", length=16)
estimated.p.values.low <- vector(mode="numeric", length=16)

for(i in 1:16)
{
  estimated.p.values.high[i] <- sum(motifs.3.random.graph[,i] > occurrency.subgraph.three.nodes.in.brc1[i])/number.randomisation
  estimated.p.values.low[i] <- sum(motifs.3.random.graph[,i] < occurrency.subgraph.three.nodes.in.brc1[i])/number.randomisation
}

significance = 0.05
indeces.significant.motifs.3.high <- which(estimated.p.values.high < significance) - 1
indeces.significant.motifs.3.low <- which(estimated.p.values.low < significance) - 1
indeces.significant.motifs.3 = c(indeces.significant.motifs.3.high, 
                                 indeces.significant.motifs.3.low)

write(x = indeces.significant.motifs.3,file = "motifs/indeces_significant_motifs_3_curveball.txt",ncolumns = 1)

## Load three nodes motifs indeces
motifs.3.ind <- read.table(file="motifs/indeces_significant_motifs_3_curveball.txt")[[1]]
...
## Motif 1 -----
occurrences.3[motifs.3.ind[i] + 1]

maps.motif.1 <- graph.subisomorphic.lad(graph.isocreate(size=3, number=motifs.3.ind[i]), 
                                        brc1.graph, all.maps=TRUE)[["maps"]]

maps.motif.1

# ... initialize list of motifs
...
write.table(x = motif.1.instances,file = "motifs/regulated_feedback_loop_20201016_curveball.tsv",quote = F,sep = "\t",row.names = F)
...
# length(strsplit(motif.1.instances[motif.1.instances[,1] == "BRC1",2],",")[[1]])
# 
# write.table(x = motif.2.instances,file = "motifs/feedforward_loop_with_multiple_output_curveball.tsv",quote = F,sep = "\t",row.names = F)
# 
# ## Motif 3 ---------
...
# length(strsplit(motif.1.instances[motif.1.instances[,1] == "BRC1",2],",")[[1]])
# 
# write.table(x = motif.3.instances,file = "motifs/feedback_loop_with_multiple_output_curveball.tsv",quote = F,sep = "\t",row.names = F)
...
for(i in 1:10)
{
  ...
  write.table(x = mean.shortest.paths,file = paste(c("results_robustness/brc1_mean_shortest_paths_",i,"_curveball.txt"),collapse=""),quote = F,row.names = F,col.names = F)
  write.table(x = number.components,file = paste(c("results_robustness/brc1_number_components_",i,"_curveball.txt"),collapse=""),quote = F,row.names = F,col.names = F)
}

dist.1 <- read.table(file="results_robustness/mean_shortest_paths_1_curveball.txt",header = F,as.is = T)[[1]]
dist.2 <- read.table(file="results_robustness/mean_shortest_paths_2_curveball.txt",header = F,as.is = T)[[1]]
dist.3 <- read.table(file="results_robustness/mean_shortest_paths_3_curveball.txt",header = F,as.is = T)[[1]]
dist.4 <- read.table(file="results_robustness/mean_shortest_paths_4_curveball.txt",header = F,as.is = T)[[1]]
dist.5 <- read.table(file="results_robustness/mean_shortest_paths_5_curveball.txt",header = F,as.is = T)[[1]]
dist.6 <- read.table(file="results_robustness/mean_shortest_paths_6_curveball.txt",header = F,as.is = T)[[1]]
dist.7 <- read.table(file="results_robustness/mean_shortest_paths_7_curveball.txt",header = F,as.is = T)[[1]]
dist.8 <- read.table(file="results_robustness/mean_shortest_paths_8_curveball.txt",header = F,as.is = T)[[1]]
dist.9 <- read.table(file="results_robustness/mean_shortest_paths_9_curveball.txt",header = F,as.is = T)[[1]]
dist.10 <- read.table(file="results_robustness/mean_shortest_paths_10_curveball.txt",header = F,as.is = T)[[1]]
...
dist.1 <- read.table(file="results_robustness/number_components_1_curveball.txt",header = F,as.is = T)[[1]]
dist.2 <- read.table(file="results_robustness/number_components_2_curveball.txt",header = F,as.is = T)[[1]]
dist.3 <- read.table(file="results_robustness/number_components_3_curveball.txt",header = F,as.is = T)[[1]]
dist.4 <- read.table(file="results_robustness/number_components_4_curveball.txt",header = F,as.is = T)[[1]]
dist.5 <- read.table(file="results_robustness/number_components_5_curveball.txt",header = F,as.is = T)[[1]]
dist.6 <- read.table(file="results_robustness/number_components_6_curveball.txt",header = F,as.is = T)[[1]]
dist.7 <- read.table(file="results_robustness/number_components_7_curveball.txt",header = F,as.is = T)[[1]]
dist.8 <- read.table(file="results_robustness/number_components_8_curveball.txt",header = F,as.is = T)[[1]]
dist.9 <- read.table(file="results_robustness/number_components_9_curveball.txt",header = F,as.is = T)[[1]]
...
dist.1 <- read.table(file="results_robustness/brc1_mean_shortest_paths_1_curveball.txt",header = F,as.is = T)[[1]]
dist.2 <- read.table(file="results_robustness/brc1_mean_shortest_paths_2_curveball.txt",header = F,as.is = T)[[1]]
dist.3 <- read.table(file="results_robustness/brc1_mean_shortest_paths_3_curveball.txt",header = F,as.is = T)[[1]]
dist.4 <- read.table(file="results_robustness/brc1_mean_shortest_paths_4_curveball.txt",header = F,as.is = T)[[1]]
dist.5 <- read.table(file="results_robustness/brc1_mean_shortest_paths_5_curveball.txt",header = F,as.is = T)[[1]]
dist.6 <- read.table(file="results_robustness/brc1_mean_shortest_paths_6_curveball.txt",header = F,as.is = T)[[1]]
dist.7 <- read.table(file="results_robustness/brc1_mean_shortest_paths_7_curveball.txt",header = F,as.is = T)[[1]]
dist.8 <- read.table(file="results_robustness/brc1_mean_shortest_paths_8_curveball.txt",header = F,as.is = T)[[1]]
dist.9 <- read.table(file="results_robustness/brc1_mean_shortest_paths_9_curveball.txt",header = F,as.is = T)[[1]]
dist.10 <- read.table(file="results_robustness/brc1_mean_shortest_paths_10_curveball.txt",header = F,as.is = T)[[1]]
...
dist.1 <- read.table(file="results_robustness/brc1_number_components_1_curveball.txt",header = F,as.is = T)[[1]]
dist.2 <- read.table(file="results_robustness/brc1_number_components_2_curveball.txt",header = F,as.is = T)[[1]]
dist.3 <- read.table(file="results_robustness/brc1_number_components_3_curveball.txt",header = F,as.is = T)[[1]]
dist.4 <- read.table(file="results_robustness/brc1_number_components_4_curveball.txt",header = F,as.is = T)[[1]]
dist.5 <- read.table(file="results_robustness/brc1_number_components_5_curveball.txt",header = F,as.is = T)[[1]]
dist.6 <- read.table(file="results_robustness/brc1_number_components_6_curveball.txt",header = F,as.is = T)[[1]]
dist.7 <- read.table(file="results_robustness/brc1_number_components_7_curveball.txt",header = F,as.is = T)[[1]]
dist.8 <- read.table(file="results_robustness/brc1_number_components_8_curveball.txt",header = F,as.is = T)[[1]]
dist.9 <- read.table(file="results_robustness/brc1_number_components_9_curveball.txt",header = F,as.is = T)[[1]]
dist.10 <- read.table(file="results_robustness/brc1_number_components_10_curveball.txt",header = F,as.is = T)[[1]]
